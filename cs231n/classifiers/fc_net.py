...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(2, self.num_layers):
        ...
        if self.use_batchnorm:
            self.params['gamma'+`i`] = np.ones(hidden_dims[i-1])
            self.params['beta'+`i`] = np.zeros(hidden_dims[i-1])
    
    self.params['W'+`self.num_layers`] = weight_scale * np.random.randn(hidden_dims[self.num_layers-2], num_classes)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        if self.use_batchnorm:
            prev_hidden, cache[i] = affine_bn_relu_forward(prev_hidden, self.params['W'+`i`], self.params['b'+`i`], 
                                                           self.params['gamma'+`i`], self.params['beta'+`i`], 
                                                           self.bn_params[i-1])
        else:
            prev_hidden, cache[i] = affine_relu_forward(prev_hidden, self.params['W'+`i`], self.params['b'+`i`])
        if self.use_dropout:
            prev_hidden, dropout_cache[i] = dropout_forward(prev_hidden, self.dropout_param)
    
    scores, cache[self.num_layers] = affine_forward(prev_hidden, self.params['W'+`self.num_layers`],
    ...
    for i in range(1, self.num_layers+1):
        sum_squares_of_w += np.sum(self.params['W'+`i`]*self.params['W'+`i`])
